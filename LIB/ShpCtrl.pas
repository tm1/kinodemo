...
type
  ...
  protected
    ...
    procedure HandleShapeStateBefore(HotTracked: Boolean; const Client: TRect; const Offset: TPoint); dynamic;
    procedure HandleShapeStateAfter(HotTracked: Boolean; const Client: TRect; const Offset: TPoint); dynamic;
  ...
  published
    ...
    property PopupMenu;
    property ShowHint;
    property TC_State: TTC_State read FTC_State write SetTC_State default scFree;
  ...
  published
    ...
    property Ticket_Repert: Integer read FTicket_Repert write SetTicket_Repert default 0;
    ...
    property Ticket_Owner: Integer read FTicket_Owner write SetTicket_Owner default 0;
    property UsedByNet: Boolean read FUsedByNet write SetUsedByNet default False;
    property OnChangeTarifpl: TNotifyEvent read FOnChangeTarifpl write FOnChangeTarifpl;
    property OnChangeUsedByNet: TNotifyEvent read FOnChangeUsedByNet write FOnChangeUsedByNet;
  end;
...
procedure TSpeedShapeControl.DrawButtonText(TextBounds: TRect; ButtonText: string; BiDiFlags: LongInt);
var
...
procedure TSpeedShapeControl.HandleShapeStateBefore(HotTracked: Boolean; const Client: TRect; const Offset: TPoint);
var
  SaveState: TTC_State;
begin
//  TTC_State = (scFree, scBroken, scReserved, scPrepared, scFixed, scFixedNoCash, scFixedCheq);
...
{      siSelected:
  ...
  begin
    ...
    if FTC_State in [scPrepared] then
      ...
      if (not (FTC_State in [scFree])) and Selected then
...
procedure TSpeedShapeControl.HandleShapeStateAfter(HotTracked: Boolean; const Client: TRect; const Offset: TPoint);
var
  wx, wy: integer;
begin
//  TTC_State = (scFree, scBroken, scReserved, scPrepared, scFixed, scFixedNoCash, scFixedCheq);
//  TEnumHS = (hsDisabled, hsHybrid, hsEnabled);
  ...
  begin
    ...
    begin
      ...
      if (FTC_State in [scFixedCheq]) and (HSettting in [hsHybrid]) then
      begin
        Canvas.Pen.Width := 0;
        ...
        // Canvas.Brush.Color := clBtnFace;
        Canvas.Rectangle(wx, (3 * Height div 4) - wy, (Width div 4) + wx, Height - wy);
      end
      else
        if (FTC_State in [scReserved, scFixedNoCash]) then
        begin
          {
...
function TSpeedShapeControl.DrawShape(const Client: TRect; const Offset: TPoint): TRect;
var
  X, Y, W, H, S: Integer;
  SaveColor: TColor;
begin
...
function TSpeedShapeControl.Draw(const Client: TRect; const Offset: TPoint): TRect;
begin
...
begin
  ...
  if FDragging then
  begin
    if not FSelected then NewState := siUp
    else NewState := siSelected;
...
begin
  ...
  begin
    ...
    begin
      ...
      if FSelected then Repaint;
    end
    else
    begin
      if FSelected then FState := siSelected;
      Repaint;
    end;
...
begin
  ...
  begin
    ...
    if Value then
    begin
      if FState = siUp then Invalidate;
      FState := siSelected
    end
    else
    begin
...
procedure TSpeedShapeControl.ActionChange(Sender: TObject; CheckDefaults: Boolean);
begin
...
begin
  if Assigned(FOnChangeTarifpl) then FOnChangeTarifpl(Self);
end;
...
begin
  if Assigned(FOnChangeUsedByNet) then FOnChangeUsedByNet(Self);
end;
