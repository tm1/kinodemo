...
uses
  ...
  procedure Register;
  begin
    RegisterComponents( 'n0mad Controls', [ TWidGetBt ] );
  end;

{ TWidGetBt }

constructor TWidGetBt.Create(AOwner: TComponent);
var
  i: integer;
begin
  ...
  for i := 1 to (AOwner as TForm).ComponentCount do
    if (AOwner as TForm).Components[i - 1] is TWidGetBt then
      inc(WidgetNumber);
...
{
function TWidGetBt.AdditionalUnits;
begin
   Result := ', WidGet';
end;
}
...
{
function TWidGetBt.CompareFirst;
begin
   Result := False;
...
(*
procedure TWidGetBt.SetupFirst(SL: TStringList; const AName,
...
begin
  ...
  begin
    RName := ParentForm.FormName + '_' + Name;
...
*)
{
procedure TWidGetBt.SetupLast(SL: TStringList; const AName,
  AParent, Prefix: String);
begin
...
{
procedure TWidGetBt.AssignEvents(SL: TStringList; const AName: String);
begin
  inherited;
...
procedure TWidGetBt.RestoreHooks(Sender: TObject);
begin
...
procedure TWidGetBt.ForceRedraw;
begin
...
procedure TWidGetBt.NewWndMethod(var msg: TMessage);
var
  hRegion: HRGN;
  dc: HDC;
  br, wr: TRect;
...
procedure TWidGetBt.CalculateWidgetArea;
var
  xframe: integer;
  yFrame: integer;
  xsize: integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (FOwnerForm.Components[i] is TWidGetBt) and (((FOwnerForm.Components[i]) as TWidGetBt).WidgetNumber < WidgetNumber) then
          Inc(TotalGap, ((FOwnerForm.Components[i]) as TWidGetBt).Gap);
      end;
    end;
...
procedure TWidGetBt.DrawWidget;
var
  WidgetState: integer;
  R: TRect;
  RB: TRect;
...
procedure TWidGetBt.Notification(AComponent: TComponent; Operation: TOperation);
begin
  if (AComponent = fPopupMenu) and (Operation = opRemove) then
    fPopupMenu := nil;
end;

procedure TWidGetBt.SetDown(Value: Boolean);
begin
  FDown := Value;
  DrawWidget;
{  Change;}
...
procedure TWidGetBt.SetEnabled(Value: boolean);
begin
...
procedure TWidGetBt.SetFont(Value: TFont);
begin
  FFont.Assign(Value);
  CalculateWidgetArea;
  DrawWidget;
...
procedure TWidGetBt.SetGap(Value: integer);
var
  i: integer;
  OwnerRect: TRect;
  hRegion: HRGN;
begin
  ...
  for i := 0 to (Owner as TForm).ComponentCount - 1 do
    if ((Owner as TForm).Components[i] is TWidGetBt) then
      (((Owner as TForm).Components[i]) as TWidGetBt).CalculateWidgetArea;
...
procedure TWidGetBt.SetGlyph(Value: char);
begin
  FGlyph := Value;
  CalculateWidgetArea;
  DrawWidget;
...
procedure TWidGetBt.SetImage(Value: TBitmap);
begin
  FImage.Assign(Value);
  DrawWidget;
{  Change;}
...
procedure TWidGetBt.SetOffsetLeft(value: integer);
begin
  FOffsetLeft := value;
  CalculateWidgetArea;
  DrawWidget;
...
procedure TWidGetBt.SetOffsetTop(value: integer);
begin
  FOffsetTop := value;
  CalculateWidgetArea;
  DrawWidget;
...
procedure TWidGetBt.SetVisible(Value: boolean);
begin
